...
bool TupleSpace::valueMatches(const Value &pattern, const Value &candidate) {
  ...
  if (pattern.type() == typeid(int64_t))
    return std::any_cast<int64_t>(pattern) == std::any_cast<int64_t>(candidate);
  if (pattern.type() == typeid(double))
    return std::any_cast<double>(pattern) == std::any_cast<double>(candidate);
  if (pattern.type() == typeid(std::string))
    return std::any_cast<std::string>(pattern) ==
           std::any_cast<std::string>(candidate);
...
bool TupleSpace::tupleMatches(const Tuple &pattern, const Tuple &candidate) {
  ...
  for (size_t i = 0; i < pattern.size(); ++i)
    if (!valueMatches(pattern[i], candidate[i]))
      return false;
  return true;
}
...
size_t TupleSpace::findRandomMatchIndexLocked(const Tuple &pattern) {
  ...
  for (size_t i = 0; i < space.size(); ++i)
    if (tupleMatches(pattern, space[i]))
      matches.push_back(i);
